...
export function TaskSidePanel({ task, updates, updatesLoading, onClose }: TaskSidePanelProps) {
  const { groups, deleteTask, moveBetweenGroups } = useTodoStore();
  ...
  const handleDelete = () => { setShowDeleteConfirm(false); deleteTask(task.id); };
  const handleMove = (toGroupId: string) => { setShowMoveModal(false); moveBetweenGroups(task.id, toGroupId); };
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-200 flex-shrink-0">
        ...
        <h2 className="flex-1 text-sm font-bold text-slate-900 truncate min-w-0">{task.title}</h2>
        <Avatar owner={task.owner} size="sm" className="flex-shrink-0" />
        <div ref={menuRef} className="relative flex-shrink-0">
          <button onClick={() => setMenuOpen((v) => !v)}
            className="p-1 rounded-md hover:bg-slate-100 text-slate-400 hover:text-slate-600 transition-colors"
            aria-label="Task actions" aria-haspopup="true" aria-expanded={menuOpen}>
      ...
      <div className="px-4 py-2.5 border-b border-slate-100 flex flex-wrap items-center gap-2 flex-shrink-0">
        ...
        {task.dueDate && <span className="text-xs text-slate-500 bg-slate-50 border border-slate-200 rounded px-2 py-0.5">Due {formatDate(task.dueDate)}</span>}
        {task.owner && <span className="text-xs text-slate-500">{task.owner.name}</span>}
        {currentGroup && (
      ...
      <div className="flex-1 overflow-y-auto">
        {activeTab === "updates" && <TaskUpdatesTab task={task} updates={updates} loading={updatesLoading} />}
        {activeTab === "activity" && <TaskActivityLogTab task={task} updates={updates} />}
      </div>
      ...
      {showMoveModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4"
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            ...
            <p className="px-5 pb-3 text-xs text-slate-400 truncate">Select a destination group for &ldquo;{task.title}&rdquo;.</p>
